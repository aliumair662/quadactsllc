...
class spoilageOrLossController extends Controller
{

    public function list()
    {
        $lists = DB::table('scrap_spoilage_loss_inventory')->orderByDesc('id')->paginate(20);
        $total = DB::table('scrap_spoilage_loss_inventory')->orderByDesc('id')->paginate(20)->sum('net_total');
        return view('spoilageLoss.list', array('lists' => $lists, 'total' => $total));
    }

    public function new()
    {
        $items = DB::table('items')->where('branch', Auth()->user()->branch)->whereIn('category', [4])->get();
        $invoice_number = DB::table('scrap_spoilage_loss_inventory')->max('id') + 1;
        return view('spoilageLoss.new', array('items' => $items, 'invoice_number' => $invoice_number));
    }
    ...
    public function store(Request $request)
    {
        ...
        $sale = DB::table('scrap_spoilage_loss_inventory')->where('voucher_number', $request->invoice_number)->first();
        if (!empty($sale)) {
            return response()->json(['success' => false, 'message' => 'Voucher already exits..', 'redirectUrl' => '/spoilageOrLoss/list'], 200);
        }
        ...
        if ($validator->fails()) {
            ...
            foreach ($items_detail as $_detail) {
                ...
                if ($company->stock_calculation == 0) {
                    ...
                    $debit = array(
                        ...
                        'general_ledger_account_id' => Config::get('constants.SPOILAGE_SCRAPE_GOODS_INVENTORY_ACCOUNT_GENERAL_LEDGER'),
            ...
            $sale = DB::table('scrap_spoilage_loss_inventory')->insert($add);
            $log = array(
                ...
                'branch' => Auth::user()->branch,
                'transaction_type' => 'Spoilage/Loss',
                'created_at' => date('Y-m-d H:i:s'),
            );
            $this->addTransactionLog($log);
            return response()->json(['success' => true, 'message' => 'Spoilage/loss added successfully..', 'redirectUrl' => '/spoilageOrLoss/list'], 200);
        }
    }

    public function edit($id)
    {
        $record = DB::table('scrap_spoilage_loss_inventory')
            ->where('id', $id)
            ->first();
        $items = DB::table('items')->get();
        return view('spoilageLoss.new', array('record' => $record, 'items' => $items));
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $sale = DB::table('scrap_spoilage_loss_inventory')->where('id', $request->id)->update($update);
            $log = array(
                'user_id' => Auth::user()->id,
                'voucher_number' => $request->invoice_number,
            ...
            $this->addTransactionLog($log);
            return response()->json(['success' => true, 'message' => 'Spoilage/loss updated successfully..', 'redirectUrl' => '/spoilageOrLoss/list'], 200);
        }
    }
    ...
    {
        $process = DB::table('scrap_spoilage_loss_inventory')->where('id', $id)->first();
        $response = array('success' => false, 'message' => '', 'redirectUrl' => '');
        $this->deleteDoubleEntry($process->voucher_number);
        DB::table('scrap_spoilage_loss_inventory')->where('voucher_number', $process->voucher_number)->delete();
        $log = array(
            'user_id' => Auth::user()->id,
        ...
        $this->addTransactionLog($log);
        return response()->json(['success' => true, 'message' => 'Spoilage/loss deleted successfully..', 'redirectUrl' => '/workinprocess/list'], 200);
    }
    ...
    public function search(Request $request)
    {
        if (isset($request->from_date) || isset($request->to_date) || isset($request->invoice_number)) {
            ...
            $query = DB::table('scrap_spoilage_loss_inventory');
            if (isset($request->invoice_number) && !empty($request->invoice_number)) {
                $query->where('scrap_spoilage_loss_inventory.voucher_number', 'like', "%$request->invoice_number%");
            } else {
                if (isset($request->from_date) && isset($request->to_date)) {
                    $query->whereBetween('scrap_spoilage_loss_inventory.voucher_date', [$request->from_date, $request->to_date]);
                }
            }
            $result = $query->orderByDesc('scrap_spoilage_loss_inventory.id')->paginate(20);
            $total = $query->orderByDesc('scrap_spoilage_loss_inventory.id')->paginate(20)->sum('net_total');
            return view('spoilageLoss.list', array('lists' => $result, 'from_date' => $request->from_date, 'to_date' => $request->to_date, 'invoice_number' => $request->invoice_number, 'total' => $total));
        } else {
            $list = DB::table('scrap_spoilage_loss_inventory')
                ->orderByDesc('scrap_spoilage_loss_inventory.id')
                ->paginate(20);
            $total = DB::table('scrap_spoilage_loss_inventory')->orderByDesc('id')->paginate(20)->sum('net_total');
            return view('spoilageLoss.list', array('lists' => $list, 'total' => $total));
        }
    }
