                    ...
                    <div class="page-title-actions">
                        <a href="{{ route('newLedgerAccount') }}">
                            <button type="button" data-toggle="tooltip" title="" data-placement="bottom"
                                class="btn-shadow mr-3 btn btn-dark" data-original-title="Add Account">
                    ...
                    <form action="{{ route('searchGeneralAccounts') }}" method="post">
                        ...
                        <div class="row no-gutters">
                            ...
                            <div class="form-group col-sm-2 mx-2">
                                ...
                                <input type="text" name="ledger_name" class="form-control" placeholder="Enter Name"
                                    value="{{ isset($searchQuery) ? $searchQuery : '' }}">
                            </div>
                            <div class="form-group col-sm-2 pl-1 pt-1 mb-0">
                                <div class="form-group">
                                    <label for="branch" class="">
                                        Search by Account Group
                                    </label>
                                    <select class="js-example-basic-single form-control"
                                        placeholder="Select Account Group" name="account_group" id="account_group">
                                        <option value="">Select Account Group</option>
                                        @if (!empty($chart_of_accounts))
                                            @foreach ($chart_of_accounts as $chart_of_account)
                                                <option value="{{ $chart_of_account->id }}"
                                                    {{ isset($searchByAccountGroup) ? ($chart_of_account->id == $searchByAccountGroup ? 'Selected' : '') : '' }}>
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <div class="mb-2 mr-2 btn-group">
                                                        ...
                                                        <div tabindex="-1" role="menu" aria-hidden="true"
                                                            ...
                                                            <a href="{{ route('ledger', $list->id) }}"><button
                                                                    type="button" tabindex="0"
                                                                    class="dropdown-item">Ledger</button></a>
                                                            @if ($list->default_account == 0 || ($list->status = 1))
                                                                <a href="{{ route('editLedgerAccount', $list->id) }}"
                                                                    class="{{ $list->default_account == 1 ? 'd-none' : '' }}"><button
                                                                        type="button" tabindex="0"
                                                                        class="dropdown-item">Edit</button></a>
                                                                <a href="{{ route('deleteLedgerAccount', $list->id) }}"
                                                                    class="{{ $list->default_account == 1 ? 'd-none' : '' }}"><button
                                                                        type="button" tabindex="0"
